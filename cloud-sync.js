...
(function(){
  ...
  async function pushSnapshot(db){
    ...
    try{
      for(const p of db.projects||[]){
        ...
        await client.from('aria_projects').upsert({id:p.id,owner_id:user.id,name:p.name,area:p.area||'general',description:p.desc||''});
      }
      ...
      for(const t of db.tasks||[]){
        ...
        if(!pid){ const localP=(db.projects||[]).find(p=>p.id===t.project); if(localP) pid=nameMap.get(localP.name); }
        const row={owner_id:user.id,title:t.title,project_id:pid||null,area:t.area||'general',description:t.description||'',due_date:t.date||null,due_time:t.time||null,priority:t.priority||'normal',status:t.status||'todo',repeat_rule:t.repeat||'none',subtasks:t.subtasks||[],attachments:t.attachments||[],drawing_url:t.drawing||null};
  ...
  function handleSignedIn(session){
    ...
    if(!user){onState('auth');return}
    onState('online');
    setTimeout(async()=>{ try{subscribe();await pull()}catch(e){console.error('ARIA sync after sign-in',e)} },0);
  }
  ...
  async function init(opts={}){
    ...
    client.auth.onAuthStateChange((event,session)=>{
      ...
      if(user) handleSignedIn(session);
      else onState('auth');
    });
  }
  ...
  async function signIn(email,password){
    ...
    try{
      ...
      return {ok:true};
    }catch(e){ return {ok:false,error:e?.message||'خطا در ورود'}; }
  }
  async function signUp(email,password){
    if(!client) await init({});
    const redirect = window.location.origin + '/';
    const {error}=await client.auth.signUp({email,password,options:{emailRedirectTo:redirect}});
  ...
  function queueFullSync(db){ clearTimeout(timer); timer=setTimeout(()=>pushSnapshot(db),500); }
