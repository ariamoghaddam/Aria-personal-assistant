(function(){
  ...
  let aiClient=null, mediaRecorder=null, chunks=[], stream=null, recording=false;
  ...
  function installHandwritingDialog(){
    ...
    function resize(){
      const r=c.getBoundingClientRect(),q=Math.max(1,window.devicePixelRatio||1);c.width=Math.round(r.width*q);c.height=Math.round(300*q);ctx.setTransform(q,0,0,q,0,0);ctx.fillStyle='#fff';ctx.fillRect(0,0,r.width,300);ctx.strokeStyle='#111';ctx.lineWidth=3;ctx.lineCap='round';ctx.lineJoin='round';
    }
    ...
    $('ariaHandClear').onclick=resize;
    $('ariaHandClose').onclick=()=>d.close();
    $('ariaHandRead').onclick=async()=>{
  ...
  async function startRecording(){
    ...
    mediaRecorder.onstop=async()=>{
      const btn=$('ariaVoiceBtn');
      try{
        const type=mediaRecorder.mimeType||chunks[0]?.type||'audio/mp4';
        ...
        stream?.getTracks?.().forEach(t=>t.stop());stream=null;mediaRecorder=null;chunks=[];recording=false;btn.disabled=false;btn.textContent='🎙 گفتن';
      }
    };
