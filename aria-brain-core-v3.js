(function(){
  ...
  function getToken(){try{const lite=localStorage.getItem('ARIA_SUPABASE_LITE_SESSION_V1');if(lite){try{const t=findToken(JSON.parse(lite));if(t)return t}catch(_){}}const preferred=`sb-${new URL(cfg.supabaseUrl).hostname.split('.')[0]}-auth-token`;const keys=[preferred,...Object.keys(localStorage).filter(k=>k.startsWith('sb-')&&k.includes('auth-token'))];for(const k of [...new Set(keys)]){const raw=localStorage.getItem(k);if(!raw)continue;try{const t=findToken(JSON.parse(raw));if(t)return t}catch(_){}}}catch(_){}throw new Error('برای هوش مصنوعی باید وارد حساب ARIA باشی.');}
