...
(async()=>{
  const marker='ARIA_SW_PURGED_FOCUS_V3';
  try{
  ...
  window.addEventListener('load',()=>setTimeout(async()=>{
    document.querySelectorAll('script[data-aria-ai-input],script[data-aria-voice-guard],script[data-aria-hand-local],script[data-aria-brain],script[data-aria-theme],script[data-aria-settings],script[data-aria-focus],script[data-aria-focus-distraction],script[data-aria-focus-analytics]').forEach(x=>x.remove());
    ...
    load('./focus-analytics.js?v=1','ariaFocusAnalytics').catch(e=>console.error('ARIA focus analytics load failed',e));
