...
(async()=>{
  const marker='ARIA_IPAD_MODULE_BOOT_V38';
  ...
  const boot=()=>setTimeout(async()=>{
    ...
    load('./settings-ui.js?v=38','ariaSettings').catch(e=>console.error('ARIA settings load failed',e));
    ...
    load('./ui-hub.js?v=38','ariaUiHub').catch(e=>console.error('ARIA command center load failed',e));
    load('./dashboard-unified.js?v=38','ariaUnifiedDash').catch(e=>console.error('ARIA unified dashboard load failed',e));
    ...
    load(fresh('./voice-fast.js?v=5'),'ariaFastVoice').catch(e=>console.error('ARIA fast voice load failed',e));
