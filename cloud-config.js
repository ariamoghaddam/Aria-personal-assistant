...
(async()=>{
  const marker='ARIA_CACHE_REFRESH_PUSH_V11';
  try{
    if(!localStorage.getItem(marker)){
      localStorage.setItem(marker,'1');
      if(window.caches){const keys=await caches.keys();await Promise.all(keys.map(k=>caches.delete(k).catch(()=>false)));}
      // مهم: Service Worker را حذف نکن؛ Push Subscription آیفون/آیپد به آن وابسته است.
    }
  }catch(e){console.warn('ARIA refresh',e)}
  ...
  window.addEventListener('load',()=>setTimeout(async()=>{
    document.querySelectorAll('script[data-aria-ai-input],script[data-aria-voice-guard],script[data-aria-hand-local],script[data-aria-brain],script[data-aria-brain-entry],script[data-aria-ai-fetch-guard],script[data-aria-notifications],script[data-aria-theme],script[data-aria-settings],script[data-aria-focus],script[data-aria-focus-distraction],script[data-aria-focus-entry],script[data-aria-productivity],script[data-aria-price-book],script[data-aria-music-pro],script[data-aria-metronome]').forEach(x=>x.remove());
    ...
    load('./notification-center.js?v=2','ariaNotifications').catch(e=>console.error('ARIA notification load failed',e));
